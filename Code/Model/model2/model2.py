...
def get_input_feature(file_name):
    ...
    if len(mfcc) < 80:
        ...
        flag = 0
        mfcc = np.resize(mfcc, (100, 13))
        return flag, mfcc
    elif len(mfcc) > 100:
        mfcc = mfcc[int(mfcc.shape[0]/2-50):int(mfcc.shape[0]/2+50)][:]
        flag = 0
...
input_one = Input(shape=(100, 13), dtype='float32', name='input_one')
...
input_two = Input(shape=(100, 13), dtype='float32', name='input_two')
...
def generate_batch(mode):
    while 1:
        train_inputs_one = np.zeros((1, 100, 13))
        train_inputs_two = np.zeros((1, 100, 13))
        ...
        for i in range(0, len(wav_label)):
            ...
            try:
                ...
                train_input_one = np.reshape(train_input_one, (1, 100, 13))
                train_input_two = np.reshape(train_input_two, (1, 100, 13))
